...
async function payIndianRiverTent(startDate, endDate, numAdults, numKids, paymentInfo) {
  ...
  const url = `https://www.campspot.com/book/indianriverrv/search/${startDateFormatted}/${endDateFormatted}/guests${numKids},${numAdults},0/list?campsiteCategory=Tent%20Sites`;
  ...
  try {
    ...
    const priceSpan = await placeContainers[0].$('span.app-average-per-night-price');
    if (!priceSpan) {
      console.log("No price span found");
      await browser.close();
      return responseData;
    }

    console.log("Price span found");
    await page.evaluate((el) => el.scrollIntoView(), placeContainers[0]);
    await new Promise(resolve => setTimeout(resolve, 500));
    await placeContainers[0].click({ delay: 100 });
    console.log("Place container clicked");
    ...
    await page.waitForSelector("button.site-booking-mobile-button.app-add-to-cart-button", 
      { visible: true, timeout: 5000 });
    ...
    await page.waitForSelector("button.site-booking-mobile-button.app-add-to-cart-button", { visible: true });
    const addToCartButton = await page.$("button.site-booking-mobile-button.app-add-to-cart-button");
    ...
    try {
      const siteLockModal = await page.waitForSelector("div.site-lock-modal-content", { timeout: 5000, visible: true });
      if (siteLockModal) {
        console.log("Site lock modal found");
        const noThanksButton = await siteLockModal.$("button.site-lock-modal-actions-button.mod-decline");
        if (noThanksButton) await noThanksButton.click();
    ...
    const creditCardRadio = await page.waitForSelector("input#payment-method-credit-card", { timeout: 10000 });
    await creditCardRadio.click();
    console.log("Credit card radio clicked");
...
if (require.main === module) {
    (async () => {
        ...
        const result = await payIndianRiverTent("06/06/25", "06/08/25", 2, 1, paymentInfo);
        console.log("Scrape result:", result);
    })();
}

module.exports = { payIndianRiverTent };