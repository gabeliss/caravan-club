...
async function payTouristParkTent(startDate, endDate, numAdults, numKids, paymentInfo, executePayment = false) {
  const responseData = {
    base_price: 0,
    tax: 0,
    total: 0,
  ...
  try {
    ...
    let addToCartButton2Found = false;
    try {
        await page.waitForSelector(".app-add-to-cart-button", { visible: true, timeout: 3000 });
        addToCartButton2Found = true;
    } catch (error) {
        console.log("Second Add to Cart button not found.");
        // const buttons = await page.$$eval('button', (btns) => btns.map(btn => btn.innerText.trim()).filter(text => text));
        // console.log("Button Texts on the Page:", buttons);
    ...
    if (addToCartButton2Found) {
        const addToCartButton2 = await page.$(".app-add-to-cart-button");
        await page.evaluate((el) => el.scrollIntoView(), addToCartButton2);
        await new Promise((resolve) => setTimeout(resolve, 1000));
        
        // Click with retry logic
        try {
            await addToCartButton2.evaluate(b => b.click());
            console.log("Second add to cart button clicked");
        } catch (error) {
            console.log("First click attempt failed for second button, trying alternative method");
            await page.evaluate((button) => button.click(), addToCartButton2);
        }
    }
