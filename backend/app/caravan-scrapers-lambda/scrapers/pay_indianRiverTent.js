...
async function payIndianRiverTent(startDate, endDate, numAdults, numKids, paymentInfo) {
  ...
  try {
    ...
    await continueToPaymentMethodButton.evaluate(b => b.click());
    ...
    try {
      ...
      await creditCardRadio.evaluate(b => b.click());
      console.log("Credit card radio clicked");
    } catch (error) {
      console.log("Credit card radio not found, printing all inputs for debugging:");
      const inputs = await page.$$('input');
      for (const input of inputs) {
        const id = await input.evaluate(el => el.id);
        const type = await input.evaluate(el => el.type);
        const name = await input.evaluate(el => el.name);
        console.log(`Input - id: ${id}, type: ${type}, name: ${name}`);
      }
      throw error;
    }
